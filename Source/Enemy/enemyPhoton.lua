...
local sprite_sheet_bala = love.graphics.newImage('Imagen/SpritesEnemys/Ph-Enemy.png')
local sprite_sheet_explosion = love.graphics.newImage('Imagen/Sprites/Explo-Bullet.png')


function EnemyPhoton:init(x, y, player, velocity)
    self.damage = 3
    self.x = x
    self.y = y
...
function EnemyPhoton:update(dt)
...
function EnemyPhoton:collides(objeto)
...
function EnemyPhoton:render()
    if self.destruible == false then
		love.graphics.draw(self.sprite, self.x, self.y)
	else
		love.graphics.draw(sprite_sheet_explosion, self.spriteExplotion, self.x - 20, self.y - 20, 0, 2, 2)
	end
end